<!DOCTYPE html>
<html lang="en">

<head>
    <!-- Required meta tags-->
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <link rel="icon" href="Kshitij_Logo.png">

    <!-- Title Page-->
    <title>Kshitij - Regstration Page</title>

    <!-- Main CSS-->
      <link href='https://fonts.googleapis.com/css?family=Raleway:200,400,800' rel='stylesheet' type='text/css'>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/normalize/5.0.0/normalize.min.css">
    <link rel='stylesheet' href='https://www.marcoguglie.it/Codepen/AnimatedHeaderBg/demo-1/css/demo.css'>    
    <link href="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/css/select2.min.css" rel="stylesheet" />
        <link rel="stylesheet" href="<?= base_url() ?>assets/css/owl.carousel.min.css">
        <link rel="stylesheet" href="<?= base_url() ?>assets/css/owl.theme.green.min.css">
    <link href="<?= base_url() ?>assets/main.css" rel="stylesheet" media="all">
</head>

<body>
<div id="large-header" class="large-header" style="overflow-x: hidden;">
    <div class="topnav">
      <a href="<?= base_url() ?>index.php/login">Home</a>
      <a href="<?= base_url() ?>sponsors/2019">Sponsors</a>
      <a href="<?= base_url() ?>index.php/team">Events</a>
      <a href="<?= base_url() ?>accomodation">Accommodation</a>
      <a href="<?= base_url() ?>notices">Notices</a>
      <a href="<?= base_url() ?>privacy">Privacy Policy</a>
      <a href="<?= base_url() ?>contact">Contact</a>
      <a href="<?= base_url() ?>index.php/logout"> Logout</a>
    </div>